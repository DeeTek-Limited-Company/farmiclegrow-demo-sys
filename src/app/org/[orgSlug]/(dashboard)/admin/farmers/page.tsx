...
export default async function AdminFarmersPage({ params, searchParams }: PageProps) {
  ...
  const [regions, districts, communities] = await Promise.all([
    ...
    prisma.district.findMany({
      where: { organizationId, ...(regionId ? { regionId } : {}) },
      include: { region: true },
  ...
  const farmerWhere: any = communityId ? { communityId, organizationId } : null;
            ...
            <CardContent className="p-6 space-y-3">
              {d.communities.map((c) => (
                <div key={c.id} className="flex items-center justify-between gap-4 rounded-2xl border border-slate-100 bg-white p-4">
                  ...
                  <Link
                    href={`${orgBase}/admin/farmers?regionId=${regionId || d.regionId}&districtId=${districtId || d.id}&communityId=${c.id}${status ? `&status=${encodeURIComponent(status)}` : ""}`}
        ...
        <Card className="border-0 shadow-2xl shadow-slate-200/50 rounded-[2.5rem] overflow-hidden">
          ...
          <CardContent className="p-6 space-y-3">
            {farmers.map((f) => (
              <Link
                key={f.id}
                href={`${orgBase}/admin/farmers/${f.id}`}
