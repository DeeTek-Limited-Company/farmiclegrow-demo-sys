...
export default async function AdminPage() {
  ...
  const results = await Promise.all([
    prisma.farmerSubmission.count({ where: { status: "PENDING_REVIEW" } }),
    prisma.farmerSubmission.count({ where: { status: "APPROVED" } }),
    prisma.farmerSubmission.count({ where: { status: "REJECTED" } }),
    ...
    prisma.productionRecord.count({ where: { status: "ACTIVE" } }),
    prisma.batch.count(),
    prisma.batch.aggregate({ _sum: { quantity: true } }),
    prisma.farmLocation.count({ where: { isValidated: false } }),
    prisma.farmer.aggregate({ _avg: { qualityScore: true } }),
    ...
    prisma.productionRecord.findMany({
      where: { status: { in: ["HARVESTED", "COMPLETED"] } },
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
        <KpiCard title="Total Farmers" value={totalFarmers.toLocaleString()} href="/admin/farmers" icon={<Users className="w-4 h-4" />} />
        <KpiCard title="Pending Reviews" value={pendingCount.toLocaleString()} href="/admin" tone="amber" icon={<AlertCircle className="w-4 h-4" />} />
        <KpiCard title="Active Cycles" value={activeCycles.toLocaleString()} href="/agronomist/production" tone="blue" icon={<TrendingUp className="w-4 h-4" />} />
