...
export default async function AgronomistPage() {
  ...
  const farmerScopeWhere = scopedDistrictIds
    ? { community: { districtId: { in: scopedDistrictIds } } }
    : {};
  ...
  const [
    ...
    prisma.farmerSubmission.findMany({
      where: user.roles.includes("admin") ? {} : { submittedById: user.id },
      include: { farmer: true },
    ...
    scopedDistrictIds
      ? prisma.district.count({ where: { id: { in: scopedDistrictIds } } })
      : prisma.district.count(),
    scopedDistrictIds
      ? prisma.district.findMany({
          where: { id: { in: scopedDistrictIds } },
          include: { region: true },
        ...
        <CardContent className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex gap-3">
            <Button asChild variant="outline" className="rounded-2xl font-black">
              <Link href="/agronomist/districts">Open districts</Link>
            </Button>
            <Button asChild variant="outline" className="rounded-2xl font-black">
              <Link href="/agronomist/locations">GPS validations</Link>
            </Button>
          </div>
        </CardContent>
      </Card>
          ...
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-8">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" asChild className="rounded-xl font-bold text-primary hover:bg-primary/5">
                <Link href="/agronomist/farmers">View All <ArrowRight className="w-4 h-4 ml-2" /></Link>
              </Button>
            </div>
          </CardHeader>
